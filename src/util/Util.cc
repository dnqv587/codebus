...
Timestamp TimeUtil::getElapse() { return Timestamp::now() - ProcessRunTime; }
...
std::string SystemUtil::BacktraceToString(int size, int skip,
...
void SystemUtil::Abort() {
  LOG_FATAL(g_logger) << "the system will exit soon, Backtrace: "
                      << BacktraceToString(64, 3);
  abort();
}

time_t SystemUtil::generateSnowflakeKey() {
  static time_t lastMS=0;
  static short sequence=0;
  const time_t currentMS=Timestamp::now().getMilliSecond();
  if (lastMS>currentMS) {
    throw std::logic_error("Occurrence time callback!");
  }else if (lastMS == currentMS) {
    constexpr short SEQUENCE_MASK = 0x1000;

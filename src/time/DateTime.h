...
public:
            ...
            :year(year),month(month),day(day)
    {
        ...
        ASSERT_THROW(month>0 && month<=12,Exception::DateTimeFormatError("in Date format,month must be between 1 and 12"));

        ASSERT_THROW(day>0 && day<=getCurrentMonthMaxDay(),Exception::DateTimeFormatError("in Date format,Number of days incorrect"));
    }
...
public:
     ...
     nanoSecond(nanoSecond)
    {
        ...
        ASSERT_THROW(nanoSecond<CONST_MAXNANOSECOND,Exception::DateTimeFormatError("in Date format,second must be less than 1,000,000,000"));
    }

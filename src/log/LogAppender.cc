...
void CheckPathName(const char* pathname)
{
	if (!isDir(pathname))
	{
		if (::mkdir(pathname, S_IROTH | S_IXOTH | S_IRGRP | S_IWGRP | S_IXGRP | S_IRUSR | S_IWUSR | S_IXUSR | S_IFDIR)
			< 0)
		{
			::perror("mdkir error!");
		}
...
AppendFile::AppendFile(std::string&& logName,
	std::string logPath,
	off64_t rollSize,
	int flushInterval,
	int flushLogCount)
...
AsynAppendFile::AsynAppendFile(std::string&& logName,
	std::string logPath,
	off_t rollSize,
	int flushInterval,
	int flushLogCount)
...
SyncAppendFile::SyncAppendFile(std::string&& logName,
	std::string logPath,
	off_t rollSize,
	int flushInterval,
	int flushLogCount) :
...
FileLogAppender::FileLogAppender(std::string logName,
	std::string logPath,
	AppenderAction action,
	int flushInterval,
	int flushLogCount,
	off64_t singleFileSize)
	: m_appendFile(action == AppenderAction::SYNC ? std::unique_ptr<AppendFile>(new SyncAppendFile(std::move(logName),
	std::move(logPath),
	singleFileSize,
	flushInterval,
	flushLogCount)) :
				   std::unique_ptr<AppendFile>(new AsynAppendFile(std::move(logName),
					   std::move(logPath),
					   singleFileSize,
					   flushInterval,
					   flushLogCount)))
...
{
	std::cout << LogColor[static_cast<unsigned char>(level)] << logStream
			  << LogColor[static_cast<unsigned char>(LogLevel::NUM_LOG_LEVELS)];
}
